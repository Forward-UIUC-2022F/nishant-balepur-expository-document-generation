...
# input / output
input_column_name = "web_sentences_no_desc"
output_dir = "/home/balepur2/baseline_results/cs history no desc/grp_full_title2.pkl"
dataset_name = 'nbalepur/cs_history_desc_refined2'

# model names
imitator_model_name = 'nbalepur/gpt2_cs_history_lg'
paraphrase_model_name = 'nbalepur/cs_history_no_desc_paraphraser_title'
retriver_model_name = 'nbalepur/cs_history_desc_bert'

# specify number of facts
k = 15

# specify the prefix (optionally by using the title/topic)
def get_prefix(topic):
    return f"{topic} was first created"
...
special_tokens_dict = {'additional_special_tokens': ['<|title|>', '<|style|>', '<|fact|>']}
...
def paraphrase(facts, style, title = ''):
    inputs = ["<|title|>" + title + "<|fact|> ".join(facts) + " <|style|> " + style]
...
def grp_model(data, idx):
    ...
    while True:
        ...
        gen_text = generate(prompt = curr_prompt, max_length = 1024, return_as_list = True)[0]
...
for idx in tqdm.tqdm(range(len(all_pred), len(test_data['title']))):
    ...
    all_pred.append((pred, fact_sentences))
